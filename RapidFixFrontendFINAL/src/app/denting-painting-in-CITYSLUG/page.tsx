...
const SERVICE_CONFIG = {
  ...
  description: "Scratch removal, body dent repair & full polish.",
  startPrice:  "₹799",
  icon:        "denting-painting" as ServiceIconKey,
};
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [
      ...
      "denting painting near me", "car denting painting near me",
    ],
...
export default async function DentingPaintingInCityPage({ searchParams }: PageProps) {
  const { city } = await searchParams;
  ...
  const content = cityContent[city] ?? {
    area: cityLabel,
    tip: "Early dent repair prevents rust and preserves your vehicle's resale value.",
    landmarks: [cityLabel],
      ...
      <section className="bg-[var(--color-primary)] text-white border-b-2 border-black">
        <div className="container mx-auto px-4 md:px-8 py-16 md:py-24 max-w-5xl">
          <div className="flex flex-col items-center gap-4 text-center md:flex-row md:items-center md:text-left md:gap-5">
            ...
            <div>
              <p className="text-sm md:text-base font-black uppercase tracking-[0.35em] text-white/80 mb-3">Denting & Painting in {cityLabel}</p>
              <h1 className="text-[clamp(3.5rem,9vw,6.5rem)] font-black uppercase tracking-tight leading-[0.92]">
                Flawless. Every Panel.
              </h1>
            </div>
          </div>
