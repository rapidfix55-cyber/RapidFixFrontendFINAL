...
const SERVICE_CONFIG = {
  slug:        "tyre-wheel",
  label:       "Tyre & Wheel",
  description: "Puncture repair, tyre replacement & wheel alignment.",
  startPrice:  "₹99",
  icon:        "tyre-wheel" as ServiceIconKey,
};
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [
      ...
      "tyre puncture repair near me", "wheel alignment near me",
    ],
...
export default async function TyreWheelInCityPage({ searchParams }: PageProps) {
  const { city } = await searchParams;
  ...
  const content = cityContent[city] ?? {
    area: cityLabel,
    tip: "Check tyre pressure monthly — correct pressure improves mileage and tyre life.",
    landmarks: [cityLabel],
      ...
      <section className="bg-[var(--color-primary)] text-white border-b-2 border-black">
        <div className="container mx-auto px-4 md:px-8 py-16 md:py-24 max-w-5xl">
          <div className="flex flex-col items-center gap-4 text-center md:flex-row md:items-center md:text-left md:gap-5">
            ...
            <div>
              <p className="text-sm md:text-base font-black uppercase tracking-[0.35em] text-white/80 mb-3">Tyre & Wheel in {cityLabel}</p>
