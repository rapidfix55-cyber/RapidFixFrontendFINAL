...
const SERVICE_CONFIG = {
  slug: "bike-service",
  label: "Bike Service",
  description: "Oil change, tune-up & regular bike maintenance.",
  startPrice: "₹299",
  icon: "bike-service" as ServiceIconKey,
};
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [
      ...
      "bike service near me", "bike repair near me"
    ],
...
export default async function BikeServiceInCityPage({ searchParams }: PageProps) {
  const { city } = await searchParams;
  ...
  const content = cityContent[city] ?? {
    area: cityLabel,
    tip: "Regular maintenance keeps your vehicle running smoothly.",
    landmarks: [cityLabel],
      ...
      <section className="bg-[var(--color-primary)] text-white border-b-2 border-black">
        <div className="container mx-auto px-4 md:px-8 py-16 md:py-24 max-w-5xl">
          <div className="flex flex-col items-center gap-4 text-center md:flex-row md:items-center md:text-left md:gap-5">
            ...
            <div>
              <p className="text-sm md:text-base font-black uppercase tracking-[0.35em] text-white/80 mb-3">Bike Service in {cityLabel}</p>
