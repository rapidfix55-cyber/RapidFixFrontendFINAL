...
const SERVICE_CONFIG = {
  slug:        "car-ac-repair",
  label:       "Car AC Repair",
  description: "AC gas refill, compressor replacement & cooling restoration.",
  startPrice:  "₹499",
  icon:        "car-ac-repair" as ServiceIconKey,
};
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [
      ...
      "car ac repair near me", "car ac service near me",
    ],
...
export default async function CarAcRepairInCityPage({ searchParams }: PageProps) {
  const { city } = await searchParams;
  ...
  const content = cityContent[city] ?? {
    area: cityLabel,
    tip: "Regular AC servicing improves fuel efficiency and keeps you cool.",
    landmarks: [cityLabel],
      ...
      <section className="bg-[var(--color-primary)] text-white border-b-2 border-black">
        <div className="container mx-auto px-4 md:px-8 py-16 md:py-24 max-w-5xl">
          <div className="flex flex-col items-center gap-4 text-center md:flex-row md:items-center md:text-left md:gap-5">
            ...
            <div>
              <p className="text-sm md:text-base font-black uppercase tracking-[0.35em] text-white/80 mb-3">Car AC Repair in {cityLabel}</p>
